...
- The run fails
- BUT prints “Undefined step” snippets you can copy into your step file
- ✅ That means: Cucumber runner found your feature file, and is asking for actors to perform the lines.
...
> [!TIP] This is your "start button".
> **`WHAT`:** It's a "`test launcher`" that tells JUnit, "Ayo! Run this Cucumber feature file broski!" (AKA a `Runner`).
>
> **It `HOOKS` to:** your `.feature` files in `src/test/resources/features`.
>
> **It is `NOT`:** a test itself for business logic; it just starts the Coocumber.

### adminLogin.feature

> [!TIP] This is your "story file"
>
> **`WHAT`:** It's a plain English checklist of behavior (AKA a `Feature File`).
>
> **It `HOOKS` to:** `step methods` inside [`AdminLoginSteps.java`](#adminloginstepsjava) (the `Step Definitions`).
>
> **It is `NOT`:** Java code, not Spring Boot code, not an API client.

### AdminLoginSteps.java

> [!TIP] This is your "translator".
> **`WHAT`:** Java methods that match the English lines in the feature file (the `Glue code` / `Step Definitions`).
>
> **It `HOOKS` to:** `RestAssured` (to actually call the API) or `Selenium` (to drive the UI), depending on the step.
>
> **It is `NOT`:** the backend code; it’s test code that drives the backend.
