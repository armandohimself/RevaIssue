...
public class IssueController {
    ...
    public Issue assignDeveloper(@PathVariable UUID issueId, @PathVariable UUID userId){
        Issue issue = issueService.getIssueById(issueId);
        User user = userService.getUserByUUID(userId);
        return issueService.assignDeveloper(issue, user);
    }
    @PutMapping("/issues/{issueId}/status")
    public Issue updateIssueStatus(@PathVariable UUID issueId, @RequestParam IssueStatus status){
        Issue issue = issueService.getIssueById(issueId);
        return issueService.updateStatus(issue, status);
    }
}
